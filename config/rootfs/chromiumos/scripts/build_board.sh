...
if [ -n "${VANILLA_MANIFEST}" ]; then
  ...
  repo init --repo-url https://chromium.googlesource.com/external/repo --manifest-url https://chromium.googlesource.com/chromiumos/manifest --manifest-name default.xml --manifest-branch ${VANILLA_MANIFEST} --groups default,bluetooth
else
  ...
  repo init --manifest-url "${KCICORE_URL}" --manifest-branch "${KCICORE_BRANCH}" --manifest-name "config/rootfs/chromiumos/cros-snapshot-$2.xml" --groups default,bluetooth
fi
